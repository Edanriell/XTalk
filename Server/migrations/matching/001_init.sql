-- Matching service: owns matching queue and match history (no cross-DB foreign keys)
...
CREATE INDEX IF NOT EXISTS idx_matching_queue_joined ON matching_queue(joined_at);
...
CREATE INDEX IF NOT EXISTS idx_match_history_user1 ON match_history(user1_id);
CREATE INDEX IF NOT EXISTS idx_match_history_user2 ON match_history(user2_id);
...
CREATE INDEX IF NOT EXISTS idx_match_history_recent ON match_history(user1_id, user2_id, created_at) WHERE created_at > NOW() - INTERVAL '24 hours';
